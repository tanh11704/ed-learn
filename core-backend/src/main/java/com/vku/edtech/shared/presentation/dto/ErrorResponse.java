...
public record ErrorResponse(
        @Schema(description = "Mã lỗi HTTP", example = "400") int status,
        @Schema(description = "Loại lỗi (Error Category)", example = "Bad Request") String error,
        @Schema(
                        description = "Thông báo lỗi chi tiết cho người dùng",
                        example = "Email đã tồn tại trong hệ thống")
                String message,
        @Schema(
                        description =
                                "Thông tin bổ sung về lỗi (thường dùng cho lỗi Validation hoặc danh sách các trường sai)",
