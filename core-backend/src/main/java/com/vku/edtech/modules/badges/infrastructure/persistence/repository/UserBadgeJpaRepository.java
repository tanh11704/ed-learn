...
public interface UserBadgeJpaRepository extends JpaRepository<UserBadgeJpaEntity, UUID> {
    ...
    @Query(
        value = "SELECT ub FROM UserBadgeJpaEntity ub JOIN FETCH ub.badge WHERE ub.user.id = :userId",
        countQuery = "SELECT COUNT(ub) FROM UserBadgeJpaEntity ub WHERE ub.user.id = :userId")
    Page<UserBadgeJpaEntity> findAllByUserIdWithBadge(@Param("userId") UUID userId, Pageable pageable);
