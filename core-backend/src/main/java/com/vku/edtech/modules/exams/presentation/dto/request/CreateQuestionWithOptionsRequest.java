...
@Schema(description = "Dữ liệu yêu cầu tạo câu hỏi kèm option lồng nhau")
public record CreateQuestionWithOptionsRequest(
        ...
        @Schema(description = "Nội dung câu hỏi")
                ...
                String content,
        @Schema(description = "Thứ tự câu hỏi") Integer orderIndex,
        @Schema(description = "Điểm câu hỏi")
                @NotNull(message = "score không được để trống")
                @Min(value = 1, message = "score phải lớn hơn 0")
                Double score,
        @Schema(description = "Đáp án chuẩn cho trả lời ngắn") String correctAnswer,
        @Schema(description = "Danh sách option lồng nhau") @Valid List<CreateOptionRequest> options) {}
