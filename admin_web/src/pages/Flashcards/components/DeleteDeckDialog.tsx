...
export default function DeleteDeckDialog({ isOpen, onClose, onConfirm, deckTitle, cardCount, isLoading }: DeleteDeckDialogProps) {
  return (
    <AnimatePresence>
      {isOpen && (
            ...
            <div className="p-6 space-y-4">
              <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
                <p className="text-red-400 text-sm font-medium">
                  Bạn có chắc chắn muốn xóa bộ flashcards <span className="font-bold">"{deckTitle}"</span>?
                </p>
                {cardCount > 0 && (
                  <p className="text-red-300/70 text-xs mt-2">
                    Bộ này chứa <span className="font-bold">{cardCount} thẻ</span>. Hành động này sẽ xóa toàn bộ nội dung.
                  </p>
                )}
                <p className="text-red-300/70 text-xs mt-2">
